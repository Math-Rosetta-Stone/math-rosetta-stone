...
const FibGame: React.FC<{ termItems: TermItem[] }> = ({ termItems }) => {
  // Use terms from context if not provided as prop

  const [hydrated, setHydrated] = useState(false);
  const [timeLeft, setTimeLeft] = useState(TIME_LIMIT);
  const [timerStopped, setTimerStopped] = useState(false);
  const [currQuestion, setCurrQuestion] = useState<TermItem | null>(
    termItems.length > 0 ? getOneRandom(termItems) : null
  );
  const [availableQuestions, setAvailableQuestions] = useState<TermItem[]>(
    termItems.length > 0 && currQuestion
  ...
  // Update question when termItems change
  useEffect(() => {
    if (termItems.length > 0 && !currQuestion) {
  ...
  return (
    <div className="flex flex-col items-center justify-start mt-2 gap-2 min-h-screen">
      ...
      <div
        ...
        {currQuestion && currQuestion.definition !== "" && (
