...
export const Matching = ({ questions, answers, answerKey, handleResetTimer, handleSubmit, handleShuffle, formSubmitted }: MatchingProps) => {
  ...
  const areAllAnswersCorrect = () => {
    return formQuestions.every((question, index) => answerKey[question] === formAnswers[index])
  }
    ...
    <DragDropContext onDragEnd={onDragEnd}>
      <div className="flex flex-col items-center">
        {formAnswers.map((answer, index) => (
          <div key={`${shuffleKey}-${index}`} className="flex flex-row justify-center items-center w-full m-5">
            ...
            <Droppable droppableId={`answer-${index}`} direction="horizontal">
              {droppableProvided => (
                ...
                >
                  <Draggable key={`answer-${index}`} index={index} draggableId={`answer-${index}`}>
                    {draggableProvided => (
                      <AnswerBox
                        answer={answer}
                        ref={
                        ...
                        variant={!formSubmitted ? "unsubmitted" : answerKey[formQuestions[index]] === answer ? "correct" : "incorrect"}
                      />
                    )}
                  </Draggable>
                </div>
        ...
        {formSubmitted && areAllAnswersCorrect() && <NextGameButton />}
      </div>
    </DragDropContext>
  )
}
