...
const TermMenu: React.FC<TermMenuProps> = ({ setCurrTermsIndex, setCurrView }) => {
    ...
    <div className="w-full">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {mockDb.map((termItem: TermItem, index: number) => (
          <label key={index} className="flex items-center bg-amber-300 text-white font-bold py-2 px-4 rounded hover:bg-amber-500 transition duration-300">
            <input
              type="checkbox"
...
const GameMenu: React.FC<{ setCurrView: React.Dispatch<React.SetStateAction<"selectTerms" | "selectGames">>}> = ( { setCurrView }) => {
  return (
    <div className="flex flex-col w-full h-full overflow-auto">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {GAMES.map((game, index) => (
          <label key={index} className="flex items-center bg-green-300 text-white font-bold py-2 px-4 rounded hover:bg-green-500 transition duration-300">
            <input
              type="checkbox"
              className="form-checkbox h-5 w-5 text-blue-600 transition duration-150 ease-in-out"
            />
